...
public class PasswordUtils {
  ...
  public static byte[] saltAndHashPassword(String password) {
    ...
    byte[] saltedHashPassword = new byte[salt.length + saltedHash.length];
    System.arraycopy(salt, 0, saltedHashPassword, 0, salt.length);
    System.arraycopy(saltedHash, 0, saltedHashPassword, salt.length, saltedHash.length);
  ...
  public static boolean plaintextMatchesSaltedHash(String plaintext, byte[] saltedHashed) {
    ...
    return Arrays.equals(hash, hashedPlaintext);
  }
